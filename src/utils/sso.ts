...
import { subBefore, getQueryMap } from "jinbi-utils";
 ...
 *
 * 注意：在 qiankun 微前端环境下，此模块不执行，避免重定向导致主应用异常
 */
(function () {
  // 在 qiankun 环境下跳过 SSO 逻辑，避免重定向导致问题
  if ((window as any).__POWERED_BY_QIANKUN__) {
    return;
  }

  // 获取 url 中的参数
  const params = getQueryMap(location.href) as Record<string, string>;
  ...
  if (sso.length === mustLength) {
    ...
    // 保存新信息到本地
    setToken(params as unknown as DataInfo<Date>);
