...
const Print = function (
  dom: string | HTMLElement | { $el: HTMLElement },
  options?: object
...
Print.prototype = {
  ...
  extendOptions: function <T extends Record<string, unknown>>(
    ...
    for (const k in obj2) {
      obj[k] = obj2[k] as T[Extract<keyof T, string>];
    }
    return obj;
  },
  /**
  ...
  getHtml: function (): Element {
    ...
    for (let k3 = 0; k3 < selects.length; k3++) {
      if (selects[k3].type == "select-one") {
        ...
        for (const option of Array.from(child)) {
  ...
  */
  ...
  /**
    Print
  */
  toPrint: function (frameWindow: Window | null): void {
