...
type IconListType = Record<string, string[]>;
...
const iconList = ref<IconListType>(IconJson as IconListType);
const icon = ref<string>("");
const currentActiveType = ref<string>("ep:");
// 深拷贝图标数据，前端做搜索
const copyIconList: IconListType = cloneDeep(iconList.value);
...
function handleClick(pane: { props: { name?: string | number } }, _ev: Event) {
  currentPage.value = 1;
  currentActiveType.value = String(pane.props.name ?? "ep:");
}
