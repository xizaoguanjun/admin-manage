import { isArray, throttle, debounce, isObject, isFunction } from "lodash-es";
  ...
  mounted(el: HTMLElement, binding: DirectiveBinding<OptimizeOptions>) {
    ...
    if (type) {
      if (value && value.event && isFunction(value.fn)) {
        ...
        const paramsArray = isArray(paramsValue)
          ? paramsValue
          : isObject(paramsValue)
            ? [paramsValue]
        ...
        useEventListener(
          ...
          type === "debounce"
            ? debounce(
                paramsArray ? () => value.fn(...paramsArray) : value.fn,
                value?.timeout ?? 200,
