...
import { ReText } from "yc-pro-components";
...
// Element Plus Icons
import {
  ArrowUpBold as ArrowUp,
  ArrowDownBold as EpArrowDown,
  ArrowLeftBold as ArrowLeft,
...
const textClass = computed(() => {
  ...
  if (isCollapse.value && !toRaw(item.meta?.icon) && item.parentId === null) {
...
<template>
  ...
  >
    ...
    >
      ...
      <el-text
        v-if="!item?.meta?.icon && isCollapse && item?.pathList?.length === 1"
        truncated
      ...
      <template #title>
        <div :style="getDivStyle">
          ...
          >
            {{ String(transformI18n(onlyOneChild?.meta?.title)) }}
          </ReText>
          <SidebarExtraIcon
            :extraIcon="String(onlyOneChild?.meta?.extraIcon ?? '')"
          />
        </div>
      </template>
    </el-menu-item>
  ...
  >
    <template #title>
      ...
      <ReText
        ...
        :class="textClass"
      >
        {{ String(transformI18n(item.meta?.title)) }}
      </ReText>
      <SidebarExtraIcon
        v-if="!isCollapse"
        :extraIcon="String(item.meta?.extraIcon ?? '')"
      />
    </template>
