...
type CompListItem = [string, Component];
...
const compList = shallowRef<CompListItem[]>([]);
...
watch(useMultiTagsStoreHook().multiTags, (tags: multiType[] | null) => {
  if (!Array.isArray(tags) || !keep.value) {
    return;
  }
  const iframeTags = tags.filter((i: multiType) => i.meta?.frameSrc);
  // tags必须是小于MAP，才是做了关闭动作，因为MAP插入的顺序在tags变化后发生
  if (iframeTags.length < MAP.size) {
    for (const i of MAP.keys()) {
      if (!tags.some((s: multiType) => s.path === i)) {
        delMap(i);
        compList.value = getMap() as CompListItem[];
      }
    }
  }
});
