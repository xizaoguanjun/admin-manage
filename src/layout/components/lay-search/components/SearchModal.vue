...
const resultRef = ref<InstanceType<typeof SearchResult> | null>(null);
...
const historyNum = getConfig().MenuSearchHistory ?? 20;
const inputRef = ref<HTMLInputElement | null>(null);

type MenuMeta = { icon?: string; title?: string };
type MenuItem = {
  path: string;
  meta?: MenuMeta;
  children?: MenuItem[];
...
function flatTree(arr: MenuItem[]): MenuItem[] {
  const res: MenuItem[] = [];
  function deep(items: MenuItem[]) {
    items.forEach(item => {
      res.push(item);
...
function search() {
  ...
  const flatMenusData = flatTree(menus as MenuItem[]);
  resultOptions.value = flatMenusData.filter((menu): menu is optionsItem =>
    keyword.value
      ? String(transformI18n(menu.meta?.title) ?? "")
          .toLocaleLowerCase()
          .includes(keyword.value.toLocaleLowerCase().trim()) ||
        (locale.value === "zh" &&
          !isAllEmpty(
            match(
              String(transformI18n(menu.meta?.title) ?? "").toLocaleLowerCase(),
...
function scrollTo(index: number) {
  const refInst = resultOptions.value.length
    ? resultRef.value
    : historyRef.value;
  const scrollTop = refInst?.handleScroll(index) ?? 0;
...
function handleEnter() {
  ...
  // 触发 changLayoutRoute 事件，让 lay-tag 组件添加标签
  emitter.emit("changLayoutRoute", options[index].path);
