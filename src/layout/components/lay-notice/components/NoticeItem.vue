...
function hoverTitle() {
  nextTick(() => {
    const scrollWidth = titleRef.value?.scrollWidth ?? 0;
    const clientWidth = titleRef.value?.clientWidth ?? 0;
    titleTooltip.value = scrollWidth > clientWidth;
  });
}
...
function hoverDescription(event: MouseEvent, description: string) {
  ...
  const cellWidth = target?.offsetWidth ?? 0;
